...
public class LoadToothUncompressedMeshFirst {
  ...
  private static void setMeshImportToTooth(Model model, List<String> logs) {
    String oldFile = "";
    try {
    ...
    if (hasTag(model.mesh().tags(), "mesh1")) {
      if (!hasTag(model.mesh("mesh1").feature().tags(), "impmsh")) {
        model.mesh("mesh1").feature().create("impmsh", "Import");
      }
      model.mesh("mesh1").feature("impmsh").set("source", "sequence");
      model.mesh("mesh1").feature("impmsh").set("sequence", "mpart1");
      try {
        model.mesh("mesh1").feature("impmsh").set("buildsource", "on");
      } catch (Exception ignored) {
      }
      try {
        model.mesh("mesh1").feature("impmsh").set("domelemsequence", "on");
      } catch (Exception ignored) {
      }
      try {
        model.mesh("mesh1").feature("impmsh").set("unmesheddom", "on");
      } catch (Exception ignored) {
      }
      model.mesh("mesh1").run("impmsh");
      logs.add("MESH_RUN|mesh1/impmsh|ok=true");
      model.mesh("mesh1").run();
      logs.add("MESH_RUN|mesh1|ok=true");
    }
  ...
  private static void runStaticStudies(Model model, List<String> logs) {
    for (String std : STUDIES) {
      ...
      try {
        model.study(std).feature("stat").set("mesh", new String[][]{{"geom1", "mesh1"}});
      } catch (Exception ignored) {
      }
  ...
  private static void runMisesPlotGroups(Model model, List<String> logs) {
  ...
  public static void main(String[] args) throws Exception {
    ...
    setMeshImportToTooth(model, logs);
    runStaticStudies(model, logs);
